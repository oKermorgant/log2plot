                      ...
                      std::map<std::string, std::string>())
{
  ...
  {
    std::cout << "axis3D already loaded" << std::endl;
  }
...
{
public:
    // default initialization with plot label, xyz-data, format and figure number

    Plot3D(const std::string& name,
           const std::vector<double>& x, const std::vector<double>& y, const std::vector<double>& z,
           const std::string& format = "",
           long fig_number = -1) {
        detail::_interpreter::get();

        // Load mpl_toolkits module if needed
        static PyObject *mpl_toolkitsmod = nullptr, *axis3dmod = nullptr;
        if (!mpl_toolkitsmod) {
          detail::_interpreter::get();
          ...
          Py_DECREF(axis3d);
          if (!axis3dmod) { throw std::runtime_error("Error loading module mpl_toolkits.mplot3d!"); }
          std::cout << "Importing mpl_tk for " << name << std::endl;
        }

        // retrieve figure or create one
        figure(fig_number);

        // create / reload 3D axis
        PyObject *gca_kwargs = PyDict_New();
        PyDict_SetItemString(gca_kwargs, "projection", PyString_FromString("3d"));
        PyObject *axis = PyObject_Call(
        ...
        label_args = PyDict_New();
        if(name != "")
            PyDict_SetItemString(label_args, "label", PyString_FromString(name.c_str()));

        // prepare plot constants
        plot_format = PyString_FromString(format.c_str());
        plot_fct = PyObject_GetAttrString(axis, "plot");
        if (!plot_fct) throw std::runtime_error("No 3D line plot");
    ...
    // shorter initialization with name or format only
    Plot3D(const std::string& name = "", const std::string& format = "", long fig_number = -1)
      : Plot3D(name, {}, {}, {}, format, fig_number) {}

    void update(const std::vector<double>& x, const std::vector<double>& y, const std::vector<double>& z) {
        assert(x.size() == y.size());
        assert(x.size() == z.size());
            // check if underlying vectors have changed
            if(x.size() != size ||
               x0 != x.data() ||y0 != y.data() ||z0 != z.data())
            {
            ...
            {
              std::cout << "update doing nothing" << std::endl;
            }
    }
    ...
    ~Plot3D() {
      if(line)
        Py_DECREF(line);
         if(label_args)
           Py_DECREF(label_args);
         if(plot_format)
           Py_DECREF(plot_format);
         if(plot_fct)
           Py_DECREF(plot_fct);
        Py_DECREF(plot_args);
    }
private:
    ...
    void plot(const std::vector<double>& x, const std::vector<double>& y, const std::vector<double>& z)
    {
      ...
      PyTuple_SetItem(plot_args, 1, yarray);
      PyTuple_SetItem(plot_args, 2, zarray);
      PyTuple_SetItem(plot_args, 3, plot_format);

      PyObject *res = PyObject_Call(plot_fct, plot_args, label_args);
    ...
    // store last plot to detect any change in xyz
    const double* x0, *y0, *z0;
    size_t size;
    PyObject* line = nullptr;
    PyObject* label_args;
    PyObject* plot_format;
    PyObject* plot_fct;
    PyObject* plot_args = PyTuple_New(4);
};
